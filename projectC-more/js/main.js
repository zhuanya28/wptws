...
function updateThree() {
  ...
  const distanceToCross = camera.position.distanceTo(cross.position);
  const maxDistanceCross = 500;
  const volumeCross = Math.max(0, 1 - distanceToCross / maxDistanceCross);

  cross.children.forEach(child => {
    if (child instanceof THREE.PositionalAudio) {
      child.setVolume(volumeCross);
      if (volumeCross > 0 && !child.isPlaying && audioContext.state === 'running') {
        child.play();
      } else if (volumeCross === 0 && child.isPlaying) {
        child.pause();
      }
    }
  });
  ...
  const distanceToSpider = camera.position.distanceTo(spider.position);
  const maxDistanceSpider = 500;
  const volumeSpider = Math.max(0, 1 - distanceToSpider / maxDistanceSpider);

  spider.children.forEach(child => {
    if (child instanceof THREE.PositionalAudio) {
      child.setVolume(volumeSpider);
      if (volumeSpider > 0 && !child.isPlaying && audioContext.state === 'running') {
        child.play();
      } else if (volumeSpider === 0 && child.isPlaying) {
        child.pause();
      }
    }
  });
...
function createCross() {
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      const sound = new THREE.PositionalAudio(listener);
      const audioLoader = new THREE.AudioLoader();
      audioLoader.load('assets/ghost_sound.mp3', (buffer) => {
        sound.setBuffer(buffer);
        sound.setRefDistance(500);
        sound.setLoop(true);
        sound.setVolume(0);
  ...
  cross.scale.set(scaleFactor, scaleFactor, scaleFactor);

  cross.position.set(x, y + scaleFactor / 2, z);
...
function createSpider() {
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      const sound = new THREE.PositionalAudio(listener);
      const audioLoader = new THREE.AudioLoader();
      audioLoader.load('assets/spider.mp3', (buffer) => {
        sound.setBuffer(buffer);
        sound.setRefDistance(500);
        sound.setLoop(true);
        sound.setVolume(0);
